import {Colors, Images} from '../theme';

// home screen dara start
const categoryListData = [
  {
    id: 1,
    heading: 'Oreeframe Row House',
    priceText: 'Rs. 1,68,000 / Year',
    bed: '4 Beds',
    baths: '4 Baths',
    sqft: '2830 sqft',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.CategoryPropartyImg,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
    badImg: Images.BedRoomIcon,
    bathsImg: Images.BathRoomIcon,
    sqftImg: Images.SqftIcon,
    isWish: false,
  },
  {
    id: 2,
    heading: 'Vatsalya Bungalows',
    priceText: 'Rs. 1,68,000 / Year',
    bed: '4 Beds',
    baths: '4 Baths',
    sqft: '2830 sqft',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.CategoryPropartyImg2,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
    badImg: Images.BedRoomIcon,
    bathsImg: Images.BathRoomIcon,
    sqftImg: Images.SqftIcon,
    isWish: false,
  },
  {
    id: 3,
    heading: 'Sakar Row House',
    priceText: 'Rs. 1,68,000 / Year',
    bed: '4 Beds',
    baths: '4 Baths',
    sqft: '2830 sqft',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.CategoryPropartyImg3,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
    badImg: Images.BedRoomIcon,
    bathsImg: Images.BathRoomIcon,
    sqftImg: Images.SqftIcon,
    isWish: false,
  },
  {
    id: 4,
    heading: 'Mahavir Row House',
    priceText: 'Rs. 1,68,000 / Year',
    bed: '4 Beds',
    baths: '4 Baths',
    sqft: '2830 sqft',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.CategoryPropartyImg4,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
    badImg: Images.BedRoomIcon,
    bathsImg: Images.BathRoomIcon,
    sqftImg: Images.SqftIcon,
    isWish: false,
  },
  {
    id: 5,
    heading: 'Siddhi Row House',
    priceText: 'Rs. 1,68,000 / Year',
    bed: '4 Beds',
    baths: '4 Baths',
    sqft: '2830 sqft',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.CategoryPropartyImg,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
    badImg: Images.BedRoomIcon,
    bathsImg: Images.BathRoomIcon,
    sqftImg: Images.SqftIcon,
    isWish: false,
  },
  {
    id: 6,
    heading: 'Suman Row House',
    priceText: 'Rs. 1,68,000 / Year',
    bed: '4 Beds',
    baths: '4 Baths',
    sqft: '2830 sqft',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.CategoryPropartyImg2,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
    badImg: Images.BedRoomIcon,
    bathsImg: Images.BathRoomIcon,
    sqftImg: Images.SqftIcon,
    isWish: false,
  },
  {
    id: 7,
    heading: 'Atlanta Row House',
    priceText: 'Rs. 1,68,000 / Year',
    bed: '4 Beds',
    baths: '4 Baths',
    sqft: '2830 sqft',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.CategoryPropartyImg3,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
    badImg: Images.BedRoomIcon,
    bathsImg: Images.BathRoomIcon,
    sqftImg: Images.SqftIcon,
    isWish: false,
  },
  {
    id: 8,
    heading: 'Swapnil Row House',
    priceText: 'Rs. 1,68,000 / Year',
    bed: '4 Beds',
    baths: '4 Baths',
    sqft: '2830 sqft',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.CategoryPropartyImg4,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
    badImg: Images.BedRoomIcon,
    bathsImg: Images.BathRoomIcon,
    sqftImg: Images.SqftIcon,
    isWish: false,
  },
];
// home screen dara end

// book screen data start
const facilitiesList = [
  {
    id: 1,
    image: Images.BedRoomIcon,
    text: 'Bedroom',
  },
  {
    id: 2,
    image: Images.BathRoomIcon,
    text: 'Bathroom',
  },
  {
    id: 3,
    image: Images.KitchenIcon,
    text: 'Kitchen',
  },
  {
    id: 4,
    image: Images.livingRoomIcon,
    text: 'living room',
  },
  {
    id: 5,
    image: Images.GarageIcon,
    text: 'Garage',
  },
  {
    id: 6,
    image: Images.KidsRoomIcon,
    text: 'Kids room',
  },
  {
    id: 7,
    image: Images.WifiIcon,
    text: 'Wifi',
  },
  {
    id: 8,
    image: Images.PoolIcon,
    text: 'Pool',
  },
  {
    id: 9,
    image: Images.PetsIcon,
    text: 'Pets',
  },
  {
    id: 10,
    image: Images.PetsIcon,
    text: 'Pets',
  },
  {
    id: 11,
    image: Images.SmokingIcon,
    text: 'Smoking',
  },
  {
    id: 12,
    image: Images.CouplesIcon,
    text: 'Couples',
  },
  {
    id: 13,
    image: Images.PetsIcon,
    text: 'Family',
  },
  {
    id: 14,
    image: Images.PetsIcon,
    text: 'Girls',
  },
];

const facilitiesCounterList = [
  {
    id: 1,
    minasImage: Images.MinasIcon,
    pluseImage: Images.PluseIcon,
    text: 'Bedrooms',
  },
  {
    id: 2,
    minasImage: Images.MinasIcon,
    pluseImage: Images.PluseIcon,
    text: 'Bathrooms',
  },
];

const propertyTypeData = [
  {
    id: 1,
    label: 'Row House',
  },
  {
    id: 2,
    label: 'Flat',
  },
  {
    id: 3,
    label: 'Appartment',
  },
  {
    id: 4,
    label: 'Villa',
  },
];

const bookPropertySubImgData = [
  {
    id: 1,
    image: Images.PopularDetailSubImg1,
  },
  {
    id: 2,
    image: Images.PopularDetailSubImg2,
  },
  {
    id: 3,
    image: Images.PopularDetailSubImg3,
  },
  {
    id: 4,
    image: Images.PopularDetailSubImg4,
  },
  {
    id: 5,
    image: Images.PopularDetailSubImg5,
  },
  {
    id: 6,
    image: Images.PopularDetailSubImg6,
  },
  {
    id: 7,
    image: Images.PopularDetailSubImg7,
  },
];

const ratingCategoryListData = [
  {
    id: 1,
    name: 'Interesting',
    width: '100%',
  },
  {
    id: 2,
    name: 'Guide',
    width: '100%',
  },
  {
    id: 3,
    name: 'Service',
    width: '90%',
  },
  {
    id: 4,
    name: 'Price',
    width: '80%',
  },
];

const reviewListData = [
  {
    id: 1,
    userImg: Images.ReviewsUserImg1,
    name: 'Stephanie',
    date: 'October, 2019',
    reviewNumber: '10',
    paregraph:
      'This is a great way to experience the Grand Canyon from Phoenix. I could never have done this trip on my own. This is a great way to experience the Grand Canyon from Phoenix.',
  },
  {
    id: 2,
    userImg: Images.ReviewsUserImg2,
    name: 'Stephanie',
    date: 'October, 2019',
    reviewNumber: '10',
    paregraph:
      'This is a great way to experience the Grand Canyon from Phoenix. I could never have done this trip on my own. This is a great way to experience the Grand Canyon from Phoenix.',
  },
  {
    id: 3,
    userImg: Images.ReviewsUserImg3,
    name: 'Stephanie',
    date: 'October, 2019',
    reviewNumber: '10',
    paregraph:
      'This is a great way to experience the Grand Canyon from Phoenix. I could never have done this trip on my own. This is a great way to experience the Grand Canyon from Phoenix.',
  },
];

const popularDestionationData = [
  {
    id: 1,
    proprtyImg: Images.HouseImageOne,
    proprtyName: 'Nomaden Sekut House',
    proceMonthYear: '$ 200 / Month',
    proprtyLocationImg: Images.grayLocationIcon,
    popularDestionationLocationText: 'San Diego, California, USA',
    proprtyRatingStarImg: Images.StarActive,
    proprtyRatingText: '4.8',
    BedRoomIcon: Images.BedRoomIcon,
    bedText: '4 Beds',
    BathRoomIcon: Images.BathRoomIcon,
    bathText: '4 Baths',
    sqftIcon: Images.SqftIcon,
    sqftText: '2830 sqft',
    likeBtnIcon: Images.Wishlist,
    chatBtnIcon: Images.ChatIcon,
    shareBtnIcon: Images.ShareFillIcon,
  },
  {
    id: 2,
    proprtyImg: Images.HouseImageOne,
    proprtyName: 'Nomaden Sekut House',
    proceMonthYear: '$ 200 / Month',
    proprtyLocationImg: Images.grayLocationIcon,
    popularDestionationLocationText: 'San Diego, California, USA',
    proprtyRatingStarImg: Images.StarActive,
    proprtyRatingText: '4.8',
    BedRoomIcon: Images.BedRoomIcon,
    bedText: '4 Beds',
    BathRoomIcon: Images.BathRoomIcon,
    bathText: '4 Baths',
    sqftIcon: Images.SqftIcon,
    sqftText: '2830 sqft',
    likeBtnIcon: Images.Wishlist,
    chatBtnIcon: Images.ChatIcon,
    shareBtnIcon: Images.ShareFillIcon,
  },
  {
    id: 3,
    proprtyImg: Images.HouseImageOne,
    proprtyName: 'Nomaden Sekut House',
    proceMonthYear: '$ 200 / Month',
    proprtyLocationImg: Images.grayLocationIcon,
    popularDestionationLocationText: 'San Diego, California, USA',
    proprtyRatingStarImg: Images.StarActive,
    proprtyRatingText: '4.8',
    BedRoomIcon: Images.BedRoomIcon,
    bedText: '4 Beds',
    BathRoomIcon: Images.BathRoomIcon,
    bathText: '4 Baths',
    sqftIcon: Images.SqftIcon,
    sqftText: '2830 sqft',
    likeBtnIcon: Images.Wishlist,
    chatBtnIcon: Images.ChatIcon,
    shareBtnIcon: Images.ShareFillIcon,
  },
  {
    id: 4,
    proprtyImg: Images.HouseImageOne,
    proprtyName: 'Nomaden Sekut House',
    proceMonthYear: '$ 200 / Month',
    proprtyLocationImg: Images.grayLocationIcon,
    popularDestionationLocationText: 'San Diego, California, USA',
    proprtyRatingStarImg: Images.StarActive,
    proprtyRatingText: '4.8',
    BedRoomIcon: Images.BedRoomIcon,
    bedText: '4 Beds',
    BathRoomIcon: Images.BathRoomIcon,
    bathText: '4 Baths',
    sqftIcon: Images.SqftIcon,
    sqftText: '2830 sqft',
    likeBtnIcon: Images.Wishlist,
    chatBtnIcon: Images.ChatIcon,
    shareBtnIcon: Images.ShareFillIcon,
  },
  {
    id: 5,
    proprtyImg: Images.HouseImageOne,
    proprtyName: 'Nomaden Sekut House',
    proceMonthYear: '$ 200 / Month',
    proprtyLocationImg: Images.grayLocationIcon,
    popularDestionationLocationText: 'San Diego, California, USA',
    proprtyRatingStarImg: Images.StarActive,
    proprtyRatingText: '4.8',
    BedRoomIcon: Images.BedRoomIcon,
    bedText: '4 Beds',
    BathRoomIcon: Images.BathRoomIcon,
    bathText: '4 Baths',
    sqftIcon: Images.SqftIcon,
    sqftText: '2830 sqft',
    likeBtnIcon: Images.Wishlist,
    chatBtnIcon: Images.ChatIcon,
    shareBtnIcon: Images.ShareFillIcon,
  },
  {
    id: 6,
    proprtyImg: Images.HouseImageOne,
    proprtyName: 'Nomaden Sekut House',
    proceMonthYear: '$ 200 / Month',
    proprtyLocationImg: Images.grayLocationIcon,
    popularDestionationLocationText: 'San Diego, California, USA',
    proprtyRatingStarImg: Images.StarActive,
    proprtyRatingText: '4.8',
    BedRoomIcon: Images.BedRoomIcon,
    bedText: '4 Beds',
    BathRoomIcon: Images.BathRoomIcon,
    bathText: '4 Baths',
    sqftIcon: Images.SqftIcon,
    sqftText: '2830 sqft',
    likeBtnIcon: Images.Wishlist,
    chatBtnIcon: Images.ChatIcon,
    shareBtnIcon: Images.ShareFillIcon,
  },
];

// book screen data end

// profile screen data start

const profileList = [
   {
    id: 9,
    image: Images.FeedBackIcon,
    text: 'Story',
    pageName: 'StoryBoard', 
  },
  {
    id: 10,
    image: Images.StarActive,
    text: 'Favourite',
    pageName: 'WishlistScreen', 
  },
  {
    id: 1,
    image: Images.BlockedUser,
    text: 'Blocked List',
    pageName: 'BlockedUser',
  },
  {
    id: 2,
    image: Images.VerifyIcon,
    text: 'Subscription',
    pageName: 'Subscription',
  },
    {
    id: 7,
    image: Images.DocumentIcon,
    text: 'Terms of Service',
    pageName: 'TermsofServiceScreen',
  },
   {
    id: 5,
    image: Images.SafetyPluseIcon,
    text: 'Safety Center',
    pageName: 'SafetyCenterScreen',
  },
  // {
  //   id: 3,
  //   image: Images.VerifyIcon,
  //   text: 'Payment Mathod',
  //   pageName: 'PaymentMathodScreen',
  // },
  {
    id: 4,
    image: Images.LockIcon,
    text: 'Change Password',
    pageName: 'ChangePasswordScreen',
  },
 
  // {
  //   id: 6,
  //   image: Images.QuestionMarkIcon,
  //   text: 'Help Center',
  //   pageName: 'HelpCenterScreen',
  // },

  // {
  //   id: 8,
  //   image: Images.FeedBackIcon,
  //   text: 'Feedback',
  //   pageName: 'FeedbackScreen',
  // },
  
  
   {
    id: 11,
    image: Images.PowerOff,
    text: 'Logout',
    pageName: 'Login', 
  },
];
const userList = [
  {
    id: 1,
    text: 'User id',
    pageName: 'UserPermissionGrant',
  },
  {
    id: 2,
    text: 'User id',
    pageName: 'UserPermissionGrant',
  },
  {
    id: 3,
    text: 'User id',
    pageName: 'UserPermissionGrant',
  },
];
const userPermissionList = [
  {
    id: 1,
    heading: 'Present Address',
    subHeadings: [
      {id: 1, title: 'Country', checked: false},
      {id: 2, title: 'State', checked: false},
      {id: 3, title: 'City', checked: false},
      {id: 4, title: 'Address', checked: false},
      {id: 5, title: 'Postal Code', checked: false},
    ],
  },
  {
    id: 2,
    heading: 'Education',
    subHeadings: [
      {id: 6, title: 'Institution', checked: false},
      {id: 7, title: 'Degree', checked: false},
    ],
  },
  {
    id: 3,
    heading: 'Career',
    subHeadings: [
      {id: 8, title: 'Job Type', checked: false},
      {id: 9, title: 'Company', checked: false},
      {id: 10, title: 'Work Area ', checked: false},
      {id: 11, title: 'Work Field', checked: false},
    ],
  },
  {
    id: 4,
    heading: 'Physical Attribute',
    subHeadings: [
      {id: 12, title: 'Height', checked: false},
      {id: 13, title: 'Weight', checked: false},
      {id: 14, title: 'Complexion ', checked: false},
      {id: 15, title: 'Blood Group', checked: false},
      {id: 16, title: 'Body Type', checked: false},
      {id: 17, title: 'Specially Abled', checked: false},
    ],
  },
  {
    id: 5,
    heading: 'Language',
    subHeadings: [
      {id: 18, title: 'Mother Tongue', checked: false},
      {id: 19, title: 'Known Language', checked: false},
    ],
  },
  {
    id: 6,
    heading: 'Hobbies & Interest',
    subHeadings: [{id: 20, title: 'Hobbies', checked: false}],
  },
  {
    id: 7,
    heading: 'Spiritual & Social Background',
    subHeadings: [
      {id: 21, title: 'Religion', checked: false},
      {id: 22, title: 'Religion Caste', checked: false},
      {id: 23, title: 'Caste ', checked: false},
      {id: 24, title: 'Sub Caste', checked: false},
      {id: 25, title: 'Family Value', checked: false},
    ],
  },
  {
    id: 8,
    heading: 'LifeStyle',
    subHeadings: [
      {id: 26, title: 'Diet', checked: false},
      {id: 27, title: 'Drink', checked: false},
      {id: 28, title: 'Smoke', checked: false},
    ],
  },
  {
    id: 9,
    heading: 'Astronomic',
    subHeadings: [
      {id: 29, title: 'Time of Birth', checked: false},
      {id: 30, title: 'City of Birth', checked: false},
      {id: 31, title: 'Manglik', checked: false},
    ],
  },
  {
    id: 10,
    heading: 'Permanant Address',
    subHeadings: [
      {id: 32, title: 'Country', checked: false},
      {id: 33, title: 'State', checked: false},
      {id: 34, title: 'City', checked: false},
      {id: 35, title: 'Address', checked: false},
      {id: 36, title: 'Postal Code', checked: false},
    ],
  },
  {
    id: 11,
    heading: 'Family Information',
    subHeadings: [
      {id: 37, title: 'Father', checked: false},
      {id: 38, title: 'Mother', checked: false},
      {id: 39, title: 'Sibiling', checked: false},
    ],
  },
];

const profileInputList = [
  {
    id: 1,
    text: 'Basic Information',
    pageName: 'ProfileDetails',
  },
  {
    id: 2,
    text: 'Gallery',
    pageName: 'AddPhoto',
  },
  {
    id: 3,
    text: 'Address',
    pageName: 'PresentAddress', 
  },
  {
    id: 4,
    text: 'Education',
    pageName: 'Education',
  },
  {
    id: 5,
    text: 'Career',
    pageName: 'Career',
  },
  {
    id: 6,
    text: 'Personality',
    pageName: 'Physical',
  },
  {
    id: 7,
    text: 'Language',
    pageName: 'Language',
  },
  {
    id: 10,
    text: 'Religious & Cultural Identity',
    pageName: 'Social',
  },
  {
    id: 11,
    text: 'Identity Verification',
    pageName: 'identityVerification',
  }
];

const chooseIdData = [
  {
    id: 1,
    label: 'Driver’s license',
  },
  {
    id: 2,
    label: 'Passport',
  },
  {
    id: 3,
    label: 'Identity card',
  },
];

const languageSelectList = [
  {
    id: 1,
    label: 'Afrikaans',
  },
  {
    id: 2,
    label: 'Bengali',
  },
  {
    id: 3,
    label: 'Chinese',
  },
  {
    id: 4,
    label: 'Dutch',
  },
  {
    id: 5,
    label: 'English',
  },
  {
    id: 6,
    label: 'French',
  },
  {
    id: 7,
    label: 'Gujarati',
  },
  {
    id: 8,
    label: 'Hindi',
  },
  {
    id: 9,
    label: 'Indonesian',
  },
  {
    id: 10,
    label: 'Japanese',
  },
  {
    id: 11,
    label: 'Kannada',
  },
  {
    id: 12,
    label: 'Latin',
  },
  {
    id: 13,
    label: 'Malayalam',
  },
  {
    id: 14,
    label: 'Nepali',
  },
  {
    id: 15,
    label: 'Odia',
  },
  {
    id: 16,
    label: 'Punjabi',
  },
  {
    id: 17,
    label: 'Romain',
  },
  {
    id: 18,
    label: 'Spanish',
  },
  {
    id: 19,
    label: 'Tamil',
  },
  {
    id: 20,
    label: 'Urdu',
  },
  {
    id: 21,
    label: 'Vietnamese',
  },
  {
    id: 22,
    label: 'Welsh',
  },
  {
    id: 23,
    label: 'Xhosa',
  },
  {
    id: 24,
    label: 'Yiddish',
  },
  {
    id: 25,
    label: 'Zulu',
  },
];

const ratingStarImgData = [
  {
    id: 1,
    image: Images.StarActive,
  },
  {
    id: 2,
    image: Images.StarActive,
  },
  {
    id: 3,
    image: Images.StarActive,
  },
  {
    id: 4,
    image: Images.StarActive,
  },
  {
    id: 5,
    image: Images.StarActive,
  },
];

const paymentOptionData = [
  {
    id: 1,
    name: 'Cards ( Credit/Debit )',
    paymentImage: Images.PaymentCardIcon,
    rightArrow: Images.ViewAllArrow,
  },
  {
    id: 2,
    name: 'EMI',
    subName: 'Credit Card, Debit Card',
    paymentImage: Images.EmiIcon,
    rightArrow: Images.ViewAllArrow,
  },
  {
    id: 3,
    name: 'Net Banking',
    paymentImage: Images.BankIcon,
    rightArrow: Images.ViewAllArrow,
  },
  {
    id: 4,
    name: 'Paytm',
    paymentImage: Images.PaytmIcon,
    rightArrow: Images.ViewAllArrow,
  },
  {
    id: 5,
    name: 'Google Pay',
    paymentImage: Images.GPayIcon,
    rightArrow: Images.ViewAllArrow,
  },
  {
    id: 6,
    name: 'UPI',
    paymentImage: Images.UPIIcon,
    rightArrow: Images.ViewAllArrow,
  },
];

const countrySelectList = [
  {
    id: 1,
    label: 'Australia',
  },
  {
    id: 2,
    label: 'Brazil',
  },
  {
    id: 3,
    label: 'Canada',
  },
  {
    id: 4,
    label: 'Denmark',
  },
  {
    id: 5,
    label: 'Ecuador',
  },
  {
    id: 6,
    label: 'France',
  },
  {
    id: 7,
    label: 'Germany',
  },
  {
    id: 8,
    label: 'Hong Kong',
  },
  {
    id: 9,
    label: 'India',
  },
  {
    id: 10,
    label: 'Japan',
  },
  {
    id: 11,
    label: 'Malaysia',
  },
  {
    id: 12,
    label: 'Netherland',
  },
  {
    id: 13,
    label: 'Philippines',
  },
  {
    id: 14,
    label: 'Spain',
  },
  {
    id: 15,
    label: 'Thailand',
  },
  {
    id: 16,
    label: 'United Kingdom',
  },
  {
    id: 17,
    label: 'Vietnam',
  },
];

const helpCenterArticleList = [
  {
    id: 1,
    name: 'Does the extenuating circumstances policy apply to my reservation during the COVID- 19 pandemic ?',
    image: Images.ViewAllArrow,
  },
  {
    id: 2,
    name: 'How do i cancel my reservation for a place to say?',
    image: Images.ViewAllArrow,
  },
  {
    id: 3,
    name: 'Verifying your identity',
    image: Images.ViewAllArrow,
  },
  {
    id: 4,
    name: 'What to do if the place you’re staying in isn’t clean at check in',
    image: Images.ViewAllArrow,
  },
];

const helpCenterTopicList = [
  {
    id: 1,
    name: 'Searching and booking',
    image: Images.SearchIcon,
  },
  {
    id: 2,
    name: 'Your reservations',
    image: Images.HouseIcon,
  },
  {
    id: 3,
    name: 'Payments, pricing, and refunds',
    image: Images.VerifyIcon,
  },
  {
    id: 4,
    name: 'Your account',
    image: Images.userRoundIcon,
  },
  {
    id: 5,
    name: 'Safety',
    image: Images.SafetyPluseIcon,
  },
  {
    id: 6,
    name: 'About Ready Rental',
    image: Images.ReadyRentalLineLogo,
  },
  {
    id: 7,
    name: 'Terms and policies',
    image: Images.infoIcon,
  },
];

const rentalPropertyListData = [
  {
    id: 1,
    heading: 'Orchad House',
    priceText: 'Rs. 1,68,000 / Year',
    bed: '4 Beds',
    baths: '4 Baths',
    sqft: '2830 sqft',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.SliderHomeHouseImageOne,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
    badImg: Images.BedRoomIcon,
    bathsImg: Images.BathRoomIcon,
    sqftImg: Images.SqftIcon,
  },
  {
    id: 2,
    heading: 'Raghuvir Silverstone',
    priceText: 'Rs. 1,68,000 / Year',
    bed: '4 Beds',
    baths: '4 Baths',
    sqft: '2830 sqft',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.CategoryPropartyImg5,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
    badImg: Images.BedRoomIcon,
    bathsImg: Images.BathRoomIcon,
    sqftImg: Images.SqftIcon,
  },
  {
    id: 3,
    heading: 'Orchad House',
    priceText: 'Rs. 1,68,000 / Year',
    bed: '4 Beds',
    baths: '4 Baths',
    sqft: '2830 sqft',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.CategoryPropartyImg6,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
    badImg: Images.BedRoomIcon,
    bathsImg: Images.BathRoomIcon,
    sqftImg: Images.SqftIcon,
  },
  {
    id: 4,
    heading: 'Raghuvir Silverstone',
    priceText: 'Rs. 1,68,000 / Year',
    bed: '4 Beds',
    baths: '4 Baths',
    sqft: '2830 sqft',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.CategoryPropartyImg7,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
    badImg: Images.BedRoomIcon,
    bathsImg: Images.BathRoomIcon,
    sqftImg: Images.SqftIcon,
  },
];

const myRentalPropertyListData = [
  {
    id: 1,
    heading: 'Orchad House',
    priceText: '$ 200 / Month ',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.SliderHomeHouseImageOne,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
  },
  {
    id: 2,
    heading: 'Raghuvir Silverstone',
    priceText: '$ 1,70 /per month',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.CategoryPropartyImg5,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
  },
  {
    id: 3,
    heading: 'Orchad House',
    priceText: '$ 200 / Month ',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.CategoryPropartyImg6,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
  },
  {
    id: 4,
    heading: 'Raghuvir Silverstone',
    priceText: '$ 1,70 /per month',
    ratingText: '4.8',
    locationAddress: 'San Diego, California, USA',
    categoryListImg: Images.CategoryPropartyImg7,
    likeImg: Images.Wishlist,
    activeIikeImg: Images.AddedWishlist,
    ratingStarImg: Images.StarActive,
  },
];

// profile screen data end

// notification screen data start

const notificationList = [
  {
    id: 1,
    heading: 'Change Password',
    time: '1 min ago',
    peregraph: 'Your app password is successfully changed by you.',
    image: Images.LockIcon,
    notRead: Colors.lightRed,
  },
  {
    id: 2,
    heading: 'Favourite',
    time: '5 hour ago',
    peregraph: '“Om Sai Row House” is in add your favorites list by you.',
    image: Images.HeartLineIcon,
    notRead: Colors.lighterGray,
  },
  {
    id: 3,
    heading: 'Message',
    time: '1 day ago',
    peregraph: '“Om Sai Row House” owner messaged you.',
    image: Images.SendGrayLineIcon,
    notRead: Colors.lighterGray,
  },
  {
    id: 4,
    heading: 'New Offer',
    time: '2 day ago',
    peregraph: 'New offer on “Aura Row House” first rent on 50% off.',
    image: Images.ReadyRentalLineLogo,
    notRead: Colors.lighterGray,
  },
  {
    id: 5,
    heading: 'New Version',
    time: '2 day ago',
    peregraph: 'Ready Rental app is available in latest version 2.11.30',
    image: Images.UpArrowRound,
    notRead: Colors.lighterGray,
  },
];

// notification screen data end

// chat screen data start

const chatUserIdData = [
  {
    id: 1,
    userImage: Images.chatUser1,
    userName: 'Orchad Row House',
    userLastSeen: 'Last seen yesterday',
  },
  {
    id: 2,
    userImage: Images.chatUser2,
    userName: 'Om Sai Row House',
    userLastSeen: 'Online',
  },
  {
    id: 3,
    userImage: Images.chatUser3,
    userName: 'Auora House',
    userLastSeen: 'Last seen 3 hours ago',
  },
  {
    id: 4,
    userImage: Images.chatUser4,
    userName: 'Oreefrem Flats',
    userLastSeen: 'Online',
  },
  {
    id: 5,
    userImage: Images.chatUser1,
    userName: 'Oberoi Villa',
    userLastSeen: 'Online',
  },
  {
    id: 6,
    userImage: Images.chatUser1,
    userName: 'Skyview Flats',
    userLastSeen: 'Last seen 30 minutes ago',
  },
];

// chat screen data end

// suggestions list data start

const suggestionsListData = [
  {
    id: 1,
    searchImg: Images.SearchIcon,
    text: 'Vip road ( vesu ), Surat',
  },
  {
    id: 2,
    searchImg: Images.SearchIcon,
    text: 'Ashok nagar',
  },
  {
    id: 3,
    searchImg: Images.SearchIcon,
    text: 'Om sai row house',
  },
  {
    id: 4,
    searchImg: Images.SearchIcon,
    text: 'Aura row house',
  },
];

// suggestions list data end

export {
  // home screen data start
  categoryListData,
  // home screen data end
  // book screen data start
  facilitiesList,
  // rulesList,
  facilitiesCounterList,
  propertyTypeData,
  bookPropertySubImgData,
  ratingCategoryListData,
  reviewListData,
  popularDestionationData,
  // book screen data end
  // profile screen data start
  profileList,
  profileInputList,
  chooseIdData,
  languageSelectList,
  ratingStarImgData,
  paymentOptionData,
  countrySelectList,
  helpCenterArticleList,
  helpCenterTopicList,
  rentalPropertyListData,
  myRentalPropertyListData,
  userList,
  userPermissionList,
  // profile screen data end
  // notification screen data start
  notificationList,
  // notification screen data end
  // chat screen data start
  chatUserIdData,
  // chat screen data end
  // suggestions list data start
  suggestionsListData,
  // suggestions list data end
};